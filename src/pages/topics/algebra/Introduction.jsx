...
export default function AlgebraIntroduction() {
  ...
  const sections = [
    ...
    },
    {
      ...
      content:
        "Solving equations is like unwrapping a present - we need to uncover what's inside!\n\nStep-by-Step Method (The Balance Approach):\n\nExample 1: x + 5 = 8\nStep 1: Identify what's added to x (it's +5)\nStep 2: Do the opposite operation to both sides (subtract 5)\nStep 3: x + 5 - 5 = 8 - 5\nStep 4: Simplify → x = 3\n\nExample 2: 2x = 10\nStep 1: Identify what's multiplying x (it's ×2)\nStep 2: Do the opposite (divide both sides by 2)\nStep 3: 2x ÷ 2 = 10 ÷ 2\nStep 4: Simplify → x = 5\n\nRemember: Whatever you do to one side, you MUST do to the other!",
      voiceText:
        "Solving equations is like unwrapping a present. We need to uncover what is inside. Example one: x plus five equals eight. Subtract five from both sides to get x equals three. Example two: two x equals ten. Divide both sides by two to get x equals five. Remember, whatever you do to one side, you must do to the other!",
      type: "problem",
      equation: "x + 5 = 8",
      solution: "3",
    ...
    },
    {
      ...
      voiceText:
        "Now it's your turn to solve. Problem: three x equals fifteen. What is the opposite operation? Apply it to both sides. Try solving it yourself before revealing the answer!",
      type: "practice",
      equation: "3x = 15",
      solution: "5",
  ...
  const startLearning = () => {
    setShowIntro(false);
    setCurrentSection(0);
    speakText(sections[0].voiceText, 300);
  };
  ...
  const handleNext = () => {
    if (currentSection < sections.length - 1) {
      ...
      resetSectionState();
      speakText(sections[nextIdx].voiceText, 300);
    }
  };
  ...
  const handlePrev = () => {
    if (currentSection > 0) {
      ...
      resetSectionState();
      speakText(sections[prevIdx].voiceText, 300);
    }
  };
  ...
  const handleSectionChange = (index) => {
    ...
    resetSectionState();
    speakText(sections[index].voiceText, 300);
  };

  const checkAnswer = () => {
    const userLower = userAnswer.trim().toLowerCase();
    const correctAnswer = sections[currentSection]?.answer?.toLowerCase() || "";
    const correctSolution = String(sections[currentSection]?.solution || "").toLowerCase();
